...
if (!empty($_POST['input_menu_validate'])) {
    ...
    if ($cek === false) {
        ...
        if (file_exists($target_file)) {
            $message = "Maaf, File Yang Anda Masukkan Telah Ada";
            $statusUpload = 0;
        } else {
            if ($_FILES['foto']['size'] > 500000) { //500kb
                $message = "File Foto Yang Anda Upload Terlalu Besar";
                $statusUpload = 0;
            } else {
    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            $message = '<script>alert("Nama menu yang anda masukkan telah ada");
                        window.location="../menu"</script>';
        } else {
